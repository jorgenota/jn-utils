...
abstract class AbstractMessageListenerContainer implements InitializingBean, DisposableBean, SmartLifecycle, BeanNameAware {
    ...
    @SuppressWarnings("FieldAccessedSynchronizedAndUnsynchronized")
    private List<SqsMessageHandler> messageHandlers;
    ...
    protected List<SqsMessageHandler> getMessageHandlers() {
        return this.messageHandlers;
    }

    public void setMessageHandlers(List<SqsMessageHandler> messageHandlers) {
     ...
     *
     * @param resourceIdResolver the resourceIdResolver to use for resolving logical to physical ids in a CloudFormation environment.
     *                           Must not be null.
     */
    ...
    private void validateConfiguration() {
        ...
        Assert.notEmpty(this.messageHandlers, "messageHandlers must not be empty");
    }
    ...
    protected void initialize() {
        synchronized (this.getLifecycleMonitor()) {
            ...
            for (SqsMessageHandler messageHandler : this.messageHandlers) {
                for (String queue : messageHandler.getChannels()) {
    ...
    private QueueAttributes queueAttributes(String queue, SqsMessageHandler messageHandler) {
        ...
        return new QueueAttributes(messageHandler, hasRedrivePolicy, destinationUrl, getMaxNumberOfMessages(), getVisibilityTimeout(), getWaitTimeOut());
    }
    ...
    protected static class QueueAttributes {
        ...
        public QueueAttributes(SqsMessageHandler messageHandler, boolean hasRedrivePolicy, String destinationUrl,
